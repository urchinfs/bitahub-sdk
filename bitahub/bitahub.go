...
func New(username, password, bitaHubUrl string, projectId int64, projectName string) (Client, error) {
	...
	if b.username == "" || b.password == "" || b.bitaHubUrl == "" || b.projectId == 0 || b.projectName == "" {
...
func parseBody(ctx context.Context, reply *Reply, body interface{}) error {
	if reply.Message.Status != types.HttpSuccess || reply.Message.Code != 0 {
		log.Printf("parseBody reply failed, error:%s", reply.Message.Message)
	...
	if body != nil {
		...
		if err != nil {
			log.Printf("parseBody json Unmarshal failed, error:%v, data:%v", err, reply.Data)
...
func (c *client) sendHttpRequest(ctx context.Context, httpMethod, httpPath string, jsonBody string, respData interface{}) error {
	for {
		...
		if httpMethod == types.HttpMethodGet {
			...
			if err != nil {
				log.Printf("GetTrainJob http request failed, error:%v", err)
				return err
			}
			...
			if err != nil {
				log.Printf("sendHttpRequest http request failed, error:%v", err)
				return err
			}
		...
		if !response.IsSuccess() {
			...
			if err == nil {
				if r.Message.Code == -1 && r.Message.Status == 200 && r.Message.Message == types.TokenExpiredMsg {
					log.Printf("token is expired, refresh token")
					c.token = ""
			...
			log.Printf("sendHttpRequest http header failed, http code:%d, inner:%s", response.StatusCode(), response.Body())
		...
		if err != nil {
			log.Printf("sendHttpRequest parse http body failed, error:%s, inner error:%s", err.Error(), r)
			return err
		}
...
func (c *client) getToken(ctx context.Context, username, password string) (string, error) {
	log.Printf("func getToken entered, username:%s", username)
	token := ""
	...
	if err != nil {
		log.Printf("GetTrainJob send http request failed, error:%s", err.Error())
		return token, err
	}

	token = resp.Token
	log.Printf("func getHfzklnToken exited, username:%s, token:%s", username, token)
	return token, nil
}
...
func (c *client) refreshToken(ctx context.Context) error {
	...
	if c.token == "" || c.tokenExpireTimestamp < nowTime {
		...
		if err != nil {
			log.Printf("Get token failed, error:%v", err)
			return err
		}
...
func (c *client) ListDirFiles(ctx context.Context, datasetName, prefix string) ([]*FileInfo, error) {
	...
	return c.listDirObjs(ctx, datasetName, prefix)
}
...
func (c *client) GetDownloadLink(ctx context.Context, datasetName, fileName string, expire time.Duration) (string, error) {
	...
	jsonBody, err := json.Marshal([]string{dirName + fileName})
	if err != nil {
		log.Printf("GetDownloadLink json Marshal failed, error:%v", err)
		return "", errors.New("internal error: json.Unmarshal fail")
	}
